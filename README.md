...
- **Elastic Enterprise Search**: App Search (7.4 to 7.6), Enterprise Search (7.7 to 8.x)
...
Available packages: `tar.gz` (Linux x86_64 / aarch64 / arm64, macOS Intel / Apple Silicon, and platform independent for App Search / Enterprise Search), `zip` (Windows 64-bit / 32-bit, and platform independent for Elasticsearch for Apache Hadoop / X-Pack), `tgz` (legacy Topbeat for macOS), `deb` (amd64 / arm64, plus legacy i386), `rpm` (x86_64 / aarch64, plus legacy i686) and `msi` (ODBC driver, 64 / 32-bit). Architecture choices are filtered per product, the version list only offers versions within the selected product's availability range (with a manual entry escape hatch for anything else), and version-specific filename schemes (e.g. Elasticsearch before 7.0.0 or Logstash before 7.10.0 not embedding the architecture) are handled automatically.
