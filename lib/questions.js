...
// The interactive prompt functions are injectable so the orchestration in
// askAnswers / askVersion can be tested without a terminal.
const DEFAULT_PROMPTS = { confirm, input, search, select };
...
function askManualVersion(defaultVersion, prompts) {
...
async function askVersion(product, versions, prompts = DEFAULT_PROMPTS) {
    const available = versions && versions.length > 0 ? versionsFor(product, versions) : [];

    if (available.length === 0) {
        return askManualVersion(undefined, prompts);
    }

    const choice = await prompts.search({
    ...
    if (choice === MANUAL_VERSION) {
        ...
        return askManualVersion(available[0], prompts);
    }
...
async function askAnswers(preset, versions, prompts = DEFAULT_PROMPTS) {
    const answers = { ...preset };

    if (answers.product === undefined) {
        answers.product = await prompts.select({
    ...
    if (answers.arch === undefined) {
        answers.arch = await prompts.select({
    ...
    if (answers.version === undefined) {
        answers.version = await askVersion(answers.product, versions, prompts);
    }

    if (answers.extract === undefined) {
        answers.extract = isExtractable(answers.arch)
            ? await prompts.confirm({ message: 'Extract the archive after the download?', default: false })
            : false;
    }
    ...
    if (answers.deleteArchive === undefined) {
        ...
        answers.deleteArchive = answers.extract === true && isExtractable(answers.arch)
            ? await prompts.confirm({ message: 'Delete the archive once extracted?', default: false })
            : false;
    }

    if (answers.confirm === undefined) {
        answers.confirm = await prompts.confirm({ message: confirmMessage(answers) });
    }
