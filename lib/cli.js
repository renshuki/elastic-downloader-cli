...
import { VERSION_FORMAT } from './versions.js';

// Read instead of importing with a JSON module attribute, which still
// prints an experimental warning on some supported Node versions.
...
// Thrown instead of exiting so main() reports it like any other error (and
// so the preset logic stays testable).
...
function presetAnswers(opts) {
    ...
    if (opts.productVersion) {
        const version = opts.productVersion.trim();

        if (!VERSION_FORMAT.test(version)) {
            fail(`Invalid version "${opts.productVersion}": versions can only contain letters, digits, dots and dashes (e.g. 8.14.0).`);
        }
    ...
    if (opts.extract || opts.deleteArchive) {
        ...
        // Fail fast instead of silently downloading without extracting.
        if (preset.arch && !isExtractable(preset.arch)) {
            fail(`--extract is not supported for .${preset.arch.ext} packages (${preset.arch.id}): only tar.gz, tgz and zip archives can be extracted.`);
        }
    }
...
export { parseArgs, presetAnswers };
